...
## 4. SYNCHRONIZATION & CIRCUIT BREAKER
- **Circuit Breaker:** Stops calling Odoo RPC if failure rate exceeds 20% and returns cached payload.
- **Webhooks & Automated Actions:** Odoo triggers webhooks to NestJS BFF on record changes.
- **Idempotency:** Webhook payload idempotency prevents duplicate record generation.
