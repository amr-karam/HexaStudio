      ...
      <Faixa as='header' className='sticky top-0 z-[var(--z-snake)] bg-transparent'>
        ...
        <nav className='hidden md:flex md:space-x-4'>
          {[['Home', 'About', 'Projects', 'Contact'].map((label) => (
            <Link key={label} href={`/${label.toLowerCase()}`} className='text-primary-100 hover:text-primary-500 font-medium transition-colors'>
              {label}
            </Link>
          ))}
        </nav>
    ...
    <HeroGradientBackground>
      <div className='relative flex flex-col items-center justify-center min-h-screen px-4 py-12 md:py-20 text-center md:text-left'>
        ...
        >
          <Button
            variant='contained'
            className='relative bg-primary-500 hover:bg-primary-600 transition-colors'
          >
            Start Exploring
          </Button>
