...
const COLORS = {
  ...
  red: 'var(--color-error)',
  redLight: '#f8717120',
  darkRed: 'var(--color-error-dark)',
  blue: 'var(--color-info)',
  blueLight: '#60a5fa20',
  violet: 'var(--color-metric-violet)',
  cyan: 'var(--color-metric-teal)',
  amber: 'var(--color-metric-amber)',
  neutral: 'var(--color-tertiary)',
  border: 'var(--color-border)',
  surface: 'var(--color-surface)',
  background: 'var(--color-void-deep)',
  textMuted: 'var(--color-tertiary)',
...
function ChartContainer({
  ...
  return (
    <div
      className={cn(
        'p-6 bg-surface border border-border rounded-2xl relative overflow-hidden',
        className,
      )}
    >
      {/* Subtle top glow */}
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-sm font-serif font-light text-foreground">{title}</h3>
          {subtitle && (
            <p className="text-[11px] text-tertiary font-light mt-0.5">{subtitle}</p>
          )}
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col items-center justify-center h-[250px] text-center">
          <div className="w-10 h-10 rounded-full bg-border flex items-center justify-center mb-3">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="var(--color-tertiary)" strokeWidth="1.5">
              <rect x="3" y="3" width="18" height="18" rx="2" />
              <line x1="3" y1="9" x2="21" y2="9" />
              <line x1="9" y1="21" x2="9" y2="9" />
            </svg>
          </div>
          <p className="text-sm text-tertiary font-light">{emptyMessage}</p>
        </div>
      ) : (
        children
      )}
...
function CustomTooltip({
  ...
  return (
    <div className="bg-surface border border-border rounded-xl px-4 py-3 shadow-[0_8px_30px_rgba(0,0,0,0.4)]">
      {label && (
        <p className="text-[10px] uppercase tracking-[0.12em] text-tertiary mb-1.5 font-medium">
          {label}
        </p>
      )}
      {payload.map((entry, i) => (
        <div key={i} className="flex items-center justify-between gap-4 py-0.5">
          <div className="flex items-center gap-2">
            ...
            />
            <span className="text-xs text-tertiary font-light">{entry.name}</span>
          </div>
...
const PIPELINE_COLORS = [
  'var(--color-info)', // blue — New
  'var(--color-metric-violet)', // indigo
  'var(--color-metric-violet)', // violet
  'var(--color-metric-violet)', // purple
  'var(--color-metric-amber)', // amber
  'var(--color-warning)', // orange
  'var(--color-gold)', // gold — Qualified
  'var(--color-success)', // emerald — Won
];
        ...
        <div className="flex items-center gap-4 mt-4 pt-4 border-t border-border flex-wrap">
          {conversionRates.map((cr, i) => (
            <div key={i} className="flex items-center gap-1.5">
              <span className="text-[10px] text-tertiary">{cr.from} → {cr.to}</span>
              <span className="text-[10px] text-gold font-medium">{cr.rate}%</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="relative w-[100px] h-[100px]">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center">
            <span className="text-xl font-serif text-foreground">{project.progress}%</span>
          </div>
        </div>
      </div>
      <p className="text-xs text-foreground font-light mt-2 text-center max-w-[100px] truncate">
        {project.projectName}
      </p>
      {project.status && (
        <span className="text-[10px] text-tertiary mt-0.5">{project.status}</span>
      )}
    </div>
  );
}
...
const DEPARTMENT_COLORS: Record<string, string> = {
  Design: 'var(--color-metric-violet)',
  Architecture: 'var(--color-info)',
  Engineering: 'var(--color-success)',
  '3D Modeling': 'var(--color-metric-amber)',
  'Project Management': 'var(--color-error)',
  Sales: 'var(--color-metric-teal)',
  Administration: 'var(--color-tertiary)',
};
      ...
      <div className="flex items-center gap-4 mt-3 pt-3 border-t border-border flex-wrap">
        {Object.entries(DEPARTMENT_COLORS).map(([dept, color]) => (
          <div key={dept} className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] text-tertiary font-light">{dept}</span>
          </div>
        ))}
      </div>
    </ChartContainer>
...
const DEFAULT_STATUS_COLORS = {
  ...
  Overdue: 'var(--color-error-dark)',
  Draft: 'var(--color-tertiary)',
  Cancelled: 'var(--color-tertiary)',
};
        ...
        {chartData.map((entry) => (
          <div key={entry.name} className="flex items-center gap-1.5">
            <span
              ...
              style={{ backgroundColor: entry.color }}
            />
            <span className="text-[10px] text-tertiary font-light">{entry.name}</span>
            <span className="text-[10px] text-foreground font-medium">{entry.value}</span>
          </div>
        ))}
      </div>
    </ChartContainer>
