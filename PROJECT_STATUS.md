...
**Last Updated:** September 03, 2026 — Completed design token alignment on branch `fix/ui-design-tokens`: created `apps/frontend/src/lib/color-tokens.ts` (canonical TypeScript color token constants mirroring CSS variables), migrated all canonical HEXA gold/void/surface/foreground hex literals across 3D components (SceneContent, ExperienceCanvas, Hotspot, XRView, ARPlacementReticle, Carousel3D, ParticleDust, ContractSignOffModal, PremiumShaders) to use named TS constants, replaced SVG `stopColor`/`stroke`/`color` attributes with `var(--color-*)` CSS variable references in ArchitecturalDataViz, ProjectWorkspaceView, RadialGlow, ContactFormSection, FAQSection, WebRtcReviewRoom, and the blog RSS route; extended `check-design-tokens.mjs` allowlist for 17 non-tokenable files (shaders, lighting/material presets, procedural canvas, collaborator cursors). Design-token gate now passes **0 violations even without `--allow-inline-style-hex`**. RSS route fixed to `force-dynamic` (SSR) to prevent build prerender timeouts from unreachable API during `next build`. All quality gates verified: ESLint 0/0, typecheck 0/0, tests 643/643, design-token gate PASSED, font preloads PASSED, `next build` clean.
**Version:** 2.2.7
