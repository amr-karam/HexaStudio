    ...
    <Html center>
      <div className="flex flex-col items-center gap-4">
        <div className="relative h-8 w-8">
          ...
          <div className="absolute inset-0 animate-ping rounded-full border border-sl-gold-subtle/30" />
        </div>
        <div className="flex flex-col items-center gap-1">
          <p className="text-sm text-sl-alabaster/60">
            {modelName ? `Loading ${modelName}...` : 'Loading 3D model...'}
          </p>
          {progress > 0 && progress < 1 && (
            <div className="h-1 w-32 overflow-hidden rounded-full bg-white/10">
              <div
                className="h-full rounded-full bg-sl-gold-subtle transition-all duration-300"
