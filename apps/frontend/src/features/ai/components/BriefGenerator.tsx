...
export function BriefGenerator() {
  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-6 md:p-12 bg-obsidian/85 backdrop-blur-2xl text-foreground border border-white/10 rounded-3xl shadow-[0_30px_90px_rgba(0,0,0,0.8),0_0_40px_rgba(212,175,55,0.08)] relative overflow-hidden">
      {/* Ambient background glow */}
      <div className="absolute top-0 right-0 w-[30rem] h-[30rem] bg-accent/5 blur-[180px] rounded-full pointer-events-none" aria-hidden="true" />
      <div className="absolute bottom-0 left-0 w-80 h-80 bg-accent/3 blur-[140px] rounded-full pointer-events-none" aria-hidden="true" />
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="w-1.5 h-1.5 rotate-45 bg-accent" aria-hidden="true" />
              <span className="text-[10px] uppercase tracking-[0.4em] text-accent font-mono">
                Gemini Spatial Intelligence
              </span>
            </div>
            <h2 className="text-3xl md:text-5xl font-serif font-light tracking-tight text-foreground">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className="text-[10px] font-mono text-text-muted uppercase tracking-widest mr-1">Presets:</span>
            {PRESET_PROGRAMS.map((preset) => (
              <button
                key={preset.label}
                type="button"
                onClick={() => setParams(preset.params)}
                className="px-3 py-1.5 rounded-full border border-white/10 hover:border-accent/40 bg-white/[0.02] hover:bg-accent/10 text-xs font-mono text-text-secondary hover:text-accent transition-all duration-300"
              >
                {preset.label}
              </button>
            ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10">
        ...
        <form onSubmit={handleGenerate} className="lg:col-span-5 flex flex-col gap-5">
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full bg-obsidian-raised border border-white/10 focus:border-accent focus:ring-1 focus:ring-accent text-foreground px-4 py-3 rounded-xl text-sm outline-none transition-all duration-300"
                required
              />
            </div>
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-obsidian-raised border border-white/10 focus:border-accent focus:ring-1 focus:ring-accent text-foreground px-4 py-3 rounded-xl text-sm outline-none font-mono transition-all duration-300"
                required
              />
            </div>
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full bg-obsidian-raised border border-white/10 focus:border-accent focus:ring-1 focus:ring-accent text-foreground p-4 rounded-xl text-sm outline-none resize-none transition-all duration-300"
                required
              />
            </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-4 mt-2 bg-accent text-background font-mono text-xs uppercase tracking-[0.25em] font-semibold rounded-xl hover:bg-accent-light disabled:opacity-40 transition-all duration-300 shadow-[0_0_25px_rgba(212,175,55,0.25)] flex items-center justify-center gap-3 cursor-pointer"
          >
            {loading ? (
              <>
              ...
              <>
                <span className="w-1.5 h-1.5 rotate-45 bg-background" aria-hidden="true" />
          ...
          <AnimatePresence mode="wait">
            {!result && !loading && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="flex flex-col items-center justify-center h-full text-center py-32 text-text-muted"
              >
                <div className="w-12 h-12 rounded-full border border-white/10 flex items-center justify-center mb-4 text-accent/50">
                ...
                <p className="text-xs text-text-muted/70 mt-2 font-light max-w-sm">
              ...
              >
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-border/20">
                  ...
                  <div className="flex items-center gap-2 self-start sm:self-center shrink-0">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg border border-border/40 hover:border-accent/60 bg-obsidian-raised text-[11px] font-mono text-text-secondary hover:text-accent transition-all duration-200"
                    >
                    ...
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg border border-accent/40 bg-accent/10 text-[11px] font-mono text-accent hover:bg-accent/20 transition-all duration-200"
                    >
                      Export JSON
                    </button>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-obsidian-raised rounded-xl border border-white/5">
                    ...
                    <p className="text-xl font-mono text-foreground font-light">
                  ...
                  <div className="p-4 bg-obsidian-raised rounded-xl border border-white/5">
                    ...
                    <p className="text-xl font-mono text-accent font-light">
                ...
                <div>
                  <h4 className="text-xs font-mono text-text-muted uppercase tracking-widest mb-3">
                  ...
                  <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
                    {result.spatialRequirements.map((req, i) => (
                      <div
                        key={i}
                        className="p-3.5 bg-obsidian-raised/80 border border-white/5 rounded-xl flex justify-between items-center hover:border-accent/20 transition-colors duration-200"
                      >
                        <div>
