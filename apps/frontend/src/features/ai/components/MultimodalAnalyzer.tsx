  ...
  return (
    <div className="w-full max-w-6xl mx-auto p-6 md:p-12 bg-sl-void text-sl-alabaster border border-sl-obsidian rounded-2xl shadow-2xl">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 pb-6 border-b border-sl-obsidian gap-4">
        <div>
          <span className="text-xs uppercase tracking-widest text-sl-gold-hover font-mono">Hermes Agent Vision</span>
        ...
        {/* Mode Selector */}
        <div className="flex flex-wrap gap-2 bg-sl-void/80 p-1.5 rounded-xl border border-sl-obsidian">
          {(['architecture', 'render', 'material', 'bim'] as AnalysisMode[]).map((m) => (
            <button
              ...
              className={`px-4 py-2 text-xs uppercase tracking-wider font-mono rounded-lg transition-all ${
                mode === m
                  ? 'bg-sl-gold-subtle text-background font-medium shadow-lg'
                  : 'text-sl-mist/60 hover:text-sl-alabaster hover:bg-sl-obsidian'
              }`}
            >
              {m === 'render' ? '3D QA' : m}
            </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-5 flex flex-col gap-6">
          <div className="border-2 border-dashed border-sl-obsidian hover:border-sl-gold-subtle/50 transition-colors rounded-2xl p-6 flex flex-col items-center justify-center text-center bg-neutral-950/50 min-h-[300px] relative overflow-hidden group">
            {imagePreview ? (
              ...
              <label className="cursor-pointer flex flex-col items-center justify-center w-full h-full py-12">
                <svg className="w-12 h-12 text-sl-mist/60 mb-4 group-hover:text-sl-gold-hover transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                <span className="text-sm font-medium text-sl-mist/80">Drop architectural render or floorplan</span>
                <span className="text-xs text-sl-mist/60 mt-1">PNG, JPG, WEBP up to 10MB</span>
          ...
          <button
            ...
            className="w-full py-4 bg-sl-gold-subtle text-background font-mono text-xs uppercase tracking-widest rounded-xl hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed transition-all shadow-xl flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
        ...
        {/* Results Column */}
        <div className="lg:col-span-7 bg-neutral-950/40 border border-sl-obsidian/80 rounded-2xl p-6 min-h-[400px] flex flex-col justify-between">
          <AnimatePresence mode="wait">
            {!archResult && !renderResult && !materialResult && !bimResult && !loading && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="flex flex-col items-center justify-center h-full text-center py-20 text-sl-mist/60"
              >
                <p className="text-sm font-mono uppercase tracking-wider">Awaiting input image and analysis trigger</p>
                <p className="text-xs text-sl-mist/60 mt-2">Select mode, upload architectural asset, and click run.</p>
              </motion.div>
            )}
            ...
            {loading && (
              <motion.div
                ...
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
                className="flex flex-col items-center justify-center h-full text-center py-20 text-sl-mist/60 gap-4"
              >
                <div className="w-10 h-10 border-2 border-sl-gold-subtle border-t-transparent rounded-full animate-spin" />
                <p className="text-xs font-mono uppercase tracking-widest text-sl-gold-hover">Extracting visual intelligence...</p>
              </motion.div>
            )}
                  ...
                  <ul className="space-y-2">
                    {archResult.suggestions.map((s, i) => (
                      <li key={i} className="text-sm text-sl-mist/80 flex items-start gap-2">
                        <span className="text-sl-gold-hover font-mono">0{i + 1}.</span> {s}
                      </li>
                    ))}
                  </ul>
                </div>
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {renderResult.improvements.map((imp, i) => (
                      <li key={i} className="text-sm text-sl-mist/80 flex items-start gap-2">
                        <span className="text-sl-gold-hover font-mono">•</span> {imp}
                      </li>
                    ))}
                  </ul>
                </div>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                    {bimResult.detectedElements.map((el, i) => (
                      <div key={i} className="p-3 bg-sl-void/50 border border-sl-obsidian rounded-xl">
                        <span className="text-xs font-medium text-sl-alabaster">{el.type}</span>
                        <div className="flex justify-between items-center mt-1 text-xs text-sl-mist/60 font-mono">
                          <span>Qty: {el.count}</span>
                          <span className="text-sl-gold-hover">{(el.confidence * 100).toFixed(0)}%</span>
                        </div>
                      </div>
                    ))}
                  </div>
