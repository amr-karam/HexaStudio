...
export function SpatialLayerToggle() {
  ...
  return (
    <>
      ...
      <button
        onClick={toggleOpen}
        className="fixed bottom-6 left-52 z-40 flex items-center space-x-2 bg-sl-void/90 hover:bg-sl-obsidian text-amber-400 border border-amber-500/30 px-3.5 py-2.5 rounded-full shadow-2xl backdrop-blur-xl text-xs font-semibold transition-all group"
      >
        <span>📐</span>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed bottom-20 left-52 z-50 w-80 bg-neutral-950/95 border border-sl-obsidian rounded-3xl p-5 text-neutral-100 shadow-2xl backdrop-blur-2xl"
          >
            {/* Header */}
            <div className="flex items-center justify-between pb-3 border-b border-sl-obsidian">
              <div>
                ...
                <p className="text-xs text-sl-mist/60">Architectural Inspection Controls</p>
              </div>
              <button onClick={toggleOpen} className="text-sl-mist/60 hover:text-neutral-200 p-1.5 rounded-lg">
                ✕
              </button>
            </div>

            {/* View Projection Switcher (2D / 3D) */}
            <div className="mt-4 p-1 bg-sl-void border border-sl-obsidian rounded-2xl flex space-x-1">
              <button
            ...
            <div className="mt-4 space-y-2 max-h-60 overflow-y-auto pr-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-sl-mist/60 block mb-1">
              ...
              {layers.map((layer) => (
                <button
                  ...
                  className={`w-full p-3 rounded-2xl border text-left transition-all flex items-center justify-between ${
                    layer.visible
                      ? 'bg-sl-void border-neutral-700 shadow-md'
                      : 'bg-sl-void/30 border-sl-obsidian/60 opacity-50'
                  }`}
                >
            ...
            {/* Opacity Slider */}
            <div className="mt-4 pt-3 border-t border-sl-obsidian space-y-1.5">
              <div className="flex justify-between text-[11px] text-sl-mist/60">
