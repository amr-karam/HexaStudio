...
export default function AboutPage() {
  return (
    <div className="bg-background text-foreground">
      ...
      <section className="relative min-h-screen flex flex-col items-center justify-center px-8 overflow-hidden">
        <div className="max-w-5xl text-center relative z-10">
          ...
          >
            ...
            <span className="italic text-accent">Meets Atmosphere</span>
          </motion.h1>
          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.2, ease: 'var(--ease-out-expo)' }}
            className="mx-auto max-w-2xl text-lg text-neutral-400 font-light leading-relaxed mb-16"
          >
            We are a boutique studio dedicated to transforming architectural data into
            emotionally resonant visual narratives. By blending cutting-edge real-time 3D
            technology with cinematic artistry, we help architects and developers present
            their vision before the foundation is laid.
          </motion.p>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.3, ease: 'var(--ease-out-expo)' }}
          >
            <Link href="/contact">
          ...
          <div className="flex flex-col lg:flex-row gap-24 items-center">
            <div className="w-full lg:w-1/2">
              ...
              <motion.p 
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.8, delay: 0.2, ease: 'var(--ease-out-expo)' }}
                className="text-neutral-400 font-light leading-relaxed text-lg mb-12"
              >
                What started as a passion for the intersection of technology and design has
                grown into a studio that partners with visionary architects worldwide. We
                believe that every building tells a story — and our craft is to tell that
            ...
            <div className="w-full lg:w-1/2 grid grid-cols-1 md:grid-cols-2 gap-8">
              {milestones.map((item, idx) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  transition={{ duration: 0.6, delay: idx * 0.1, ease: 'var(--ease-out-expo)' }}
                  className="p-8 border-l border-border hover:border-accent transition-colors duration-500"
                >
                  <span className="text-2xl font-serif text-accent font-light">
                    {item.year}
                  </span>
                  <p className="text-xs uppercase tracking-widest text-foreground mt-4 mb-2 font-medium">
                    {item.label}
                  </p>
      ...
      {/* Values Section */}
      <section className="px-8 md:px-16 py-32 bg-surface relative overflow-hidden">
        ...
        <div className="max-w-screen-2xl mx-auto relative z-10">
          ...
          >
            ...
            <h2 className="text-5xl md:text-7xl font-serif font-light tracking-tight text-foreground">
              What Drives <span className="italic text-accent">Us</span>
            </h2>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {values.map((value, idx) => (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ duration: 0.6, delay: idx * 0.1, ease: 'var(--ease-out-expo)' }}
                className="p-12 bg-background/50 backdrop-blur-sm border border-border hover:border-accent/30 transition-all duration-500 group"
              >
                <span className="text-3xl font-serif text-accent/50 group-hover:text-accent transition-colors duration-500 font-light">
                  {String(idx + 1).padStart(2, '0')}
                </span>
                <h3 className="text-sm uppercase tracking-[0.3em] text-foreground mt-8 mb-4">
                  {value.title}
                </h3>
                <p className="text-xs text-neutral-500 font-light leading-relaxed">
                  {value.description}
