...
Forecast timeline with milestones.`,
      ...
      this.logger.error(`Timeline forecast failed: ${(error as Error).message}`);
      return this.fallbackForecast();
    }
  }
...
Optimize allocation.`,
      ...
      this.logger.error(`Resource optimization failed: ${(error as Error).message}`);
...
Forecast budget.`,
      ...
      this.logger.error(`Budget forecast failed: ${(error as Error).message}`);
